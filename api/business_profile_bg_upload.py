...
DESTINATION_DIR = '/business_profile_bg'
UPLOAD_DIR = app.config['media'] + DESTINATION_DIR

@app.route('/business_profile_bg_upload', methods=['POST'])
def business_profile_bg_upload():
    try:
        # Get form data
        ...
        cursor.execute(
            "SELECT * FROM tbl_business_profile_bg WHERE user_guid = %s AND business_guid = %s",
            (guid, bid)
        )
        ...
        if not existing:
            # Insert new record
            cursor.execute(
                """INSERT INTO tbl_business_profile_bg 
            ...
            cursor.execute(
                """UPDATE tbl_business_profile_bg 
...
@app.route('/vmedia/business_profile_bg/<string:filename>')
def serve_business_profile_bg(filename):
